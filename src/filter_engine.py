...
Correções Qwen (Arquivo 6):
- Unificação da Blacklist: Adicionado suporte a `data/blacklist.txt` como fonte
  primária e dinâmica de bloqueios, sem remover as defesas hardcoded/DB.
...
class FilterEngine:
    ...
    # Mantido como defesa hardcoded (Regra de Preservação).
    EXCLUDED_SENDERS = [
        "lixo@didier.com.br",
    ...
    def __init__(self):
        ...
        # Nova camada de filtro: Blacklist unificada via arquivo
        self._blacklist_file_emails = set()
        ...
        try:
            with open(baseline_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    name, email_addr = parseaddr(line)
                    if email_addr and "@" in email_addr:
                        self._baseline_emails.add(email_addr.lower())
                    else:
            ...
            print(f"[FILTER WARN] Erro ao ler baseline: {e}")

    def _load_blacklist_file(self) -> None:
        """
        Carrega a blacklist unificada do arquivo data/blacklist.txt.
        Esta é a fonte primária recomendada para bloqueios de remetentes,
        permitindo atualização dinâmica sem alterar o código ou reiniciar o serviço.
        """
        blacklist_path = _PROJECT_ROOT / "data" / "blacklist.txt"
            ...
            with open(blacklist_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if line and not line.startswith("#"):
                        self._blacklist_file_emails.add(line.lower())
        except Exception as e:
            print(f"[FILTER WARN] Erro ao ler {blacklist_path}: {e}")
    ...
    def evaluate(
        ...
        Decisoes: 
          ...
          - SKIP_BLACKLIST (remetente na blacklist do DB ou arquivo)
        ...
        # 1. Blacklist do DB (tem prioridade maxima - bloqueio manual via UI/API)
        if db.is_blacklisted(sender_email):
            return "SKIP_BLACKLIST", f"Remetente na blacklist do DB: {sender_email}", None

        # 1.5 Blacklist do Arquivo (data/blacklist.txt - fonte unificada)
        if sender_email in self._blacklist_file_emails:
            return "SKIP_BLACKLIST", f"Remetente na blacklist (arquivo): {sender_email}", None
        ...
        # 4. Remetente na lista de exclusao hardcoded (ruido conhecido)
        if sender_email in self.EXCLUDED_SENDERS:
            return "SKIP_SENDER", f"Remetente excluido (hardcoded): {sender_email}", None
    ...
    def get_baseline_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "blacklist_file": len(self._blacklist_file_emails),
        }
...
if __name__ == "__main__":
    ...
    print(f"Blacklist (arquivo): {stats['blacklist_file']} emails bloqueados.")
    print("-" * 70)
